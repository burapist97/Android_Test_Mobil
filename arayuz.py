...
class TestOtomasyonApp(ctk.CTk):
    ...
    def goster_kayit(self):
        ...
        uyari_frame = ctk.CTkFrame(self.main_frame, fg_color="#3a2f2f", corner_radius=8)
        uyari_frame.pack(pady=5, padx=20, fill="x")
        ctk.CTkLabel(uyari_frame, text="⚠️ DİKKAT EDİLMESİ GEREKENLER", font=("Arial", 14, "bold"), text_color="#ffcc00").pack(pady=(5,0))
        uyari_metni = ("1- Yaratılan test sadece yaratıldığı telefonda tekrar oynatılabilmektedir.\n"
                       "2- Lütfen testlerinizin daha sürdürülebilir olması için teste başlangıç ve bitiş noktalarını aynı yapınız.")
        ctk.CTkLabel(uyari_frame, text=uyari_metni, font=("Arial", 12), text_color="#ffcc00", justify="left").pack(pady=(0,5), padx=10)
        ...
        self.entry_telefon = ctk.CTkEntry(self.main_frame, placeholder_text="Telefon Modeli (Örn: Samsung S23)", width=300)
        self.entry_telefon.pack(pady=5)
        self.entry_notlar = ctk.CTkEntry(self.main_frame, placeholder_text="Teste Özel Notlar / Uyarılar", width=300)
        self.entry_notlar.pack(pady=5)
        ...
        self.log_kutusu = ctk.CTkTextbox(self.main_frame, width=600, height=180)
        self.log_kutusu.pack(pady=5)
    ...
    def kaydi_tetikle(self):
        ...
        self.guncel_notlar = self.entry_notlar.get()
    ...
    def testi_disa_aktar(self, test_adi):
        try:
            conn = sqlite3.connect(self.db_yolu)
            cursor = conn.cursor()
            cursor.execute("SELECT yetkili, uygulama, amac, case_adi, aksiyonlar, beklenen_xml, telefon_modeli, notlar FROM case_bazli_testler WHERE ana_test_adi = ? ORDER BY id ASC", (test_adi,))
            satirlar = cursor.fetchall()
            conn.close()
            if not satirlar: return
            
            t_model = satirlar[0][6] if len(satirlar[0]) > 6 and satirlar[0][6] is not None else ""
            t_notlar = satirlar[0][7] if len(satirlar[0]) > 7 and satirlar[0][7] is not None else ""
            
            export_data = {
                "ana_test_adi": test_adi, "yetkili": satirlar[0][0], "uygulama": satirlar[0][1], "amac": satirlar[0][2],
                "telefon_modeli": t_model, "notlar": t_notlar, "caseler": []
            }
    ...
    def testi_ice_aktar(self):
        ...
        try:
            ...
            t_model = data.get("telefon_modeli", "")
            t_notlar = data.get("notlar", "")
            ...
            for case in data.get("caseler", []):
                cursor.execute("""INSERT INTO case_bazli_testler 
                               (ana_test_adi, yetkili, uygulama, amac, case_adi, aksiyonlar, beklenen_xml, telefon_modeli, notlar) 
                               VALUES (?,?,?,?,?,?,?,?,?)""", 
                               (ana_test_adi, yetkili, uygulama, amac, case["case_adi"], case["aksiyonlar"], case["beklenen_xml"], t_model, t_notlar))
            conn.commit()
            conn.close()
            self.listeyi_guncelle()
    ...
    def testi_oynat(self, test_adi):
        ...
        try:
            conn = sqlite3.connect(self.db_yolu)
            cursor = conn.cursor()
            cursor.execute("SELECT telefon_modeli, notlar FROM case_bazli_testler WHERE ana_test_adi = ? LIMIT 1", (test_adi,))
            ek_bilgiler = cursor.fetchone()
            conn.close()
            t_model = ek_bilgiler[0] if ek_bilgiler and len(ek_bilgiler) > 0 and ek_bilgiler[0] else "Bilinmiyor"
            t_notlar = ek_bilgiler[1] if ek_bilgiler and len(ek_bilgiler) > 1 and ek_bilgiler[1] else "Yok"
        except Exception:
            t_model = "Bilinmiyor"
            t_notlar = "Yok"

        info_frame = ctk.CTkFrame(self.img_frame, fg_color="#3a2f2f", corner_radius=5)
        info_frame.pack(fill="x", pady=(0,10))
        ctk.CTkLabel(info_frame, text=f"📱 Cihaz Modeli: {t_model}   |   ⚠️ Notlar: {t_notlar}", font=("Arial", 13, "bold"), text_color="#ffcc00").pack(pady=5, padx=10)
            ...
            for dongu in range(1, tekrar_sayisi + 1):
                ...
                for c_adi, aksiyonlar, ref_veri in cases:
                    ...
                    if ref_veri.endswith(".png"):
                        ...
                        if os.path.exists(referans_tam_yol):
                            max_deneme = 8
                            basarili = False
                            skor_yuzde_son = 0
                            isaretli_foto_yol_son = None
                            ...
                            for deneme in range(max_deneme):
                                ...
                                if os.path.exists(test_foto_yol):
                                    ...
                                    if deneme < max_deneme - 1:
                                        ekrana_yaz(f"⏳ Ekran yükleniyor (%{skor_yuzde}). Tekrar deneniyor... ({deneme+1}/{max_deneme})")
                                        time.sleep(1.5)
                                else:
                                    break
                                    
                            if not basarili:
                                ekrana_yaz(f"❌ BAŞARISIZ! (Benzerlik: %{skor_yuzde_son})\n" + "-"*35)
                                ...
                                if isaretli_foto_yol_son and os.path.exists(isaretli_foto_yol_son):
                                    os.rename(isaretli_foto_yol_son, foto_yol) 
                                adim_raporlari.append(f"❌ {c_adi} - BAŞARISIZ (Benzerlik: %{skor_yuzde_son}) | IMG:{foto_yol}")
                                ...
                                ekrana_yaz(f"✅ BAŞARILI! (Benzerlik: %{skor_yuzde_son})\n" + "-"*35)
                                basarili_adim += 1
                                adim_raporlari.append(f"✅ {c_adi} - BAŞARILI (Benzerlik: %{skor_yuzde_son})")
                                if isaretli_foto_yol_son and os.path.exists(isaretli_foto_yol_son): os.remove(isaretli_foto_yol_son) 
                                if os.path.exists(test_foto_yol): os.remove(test_foto_yol) 
                        else:
