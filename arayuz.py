...
class TestOtomasyonApp(ctk.CTk):
    ...
    def kaydi_tetikle(self):
        ...
        self.log_yaz("👉 Tüm Tıklama ve Kaydırmaları (Swipe) direkt cihaz ekranından doğal şekilde yapın!\n👉 Adım bittiğinde bilgisayardan ENTER tuşuna basın.\n")
    ...
    def klavye_dinle(self, key):
        if not self.kayit_aktif or self.popup_acik: return 
        if key == Key.enter: self.after(0, self.adim_ismi_sor_ve_kaydet)
        elif key == Key.esc: self.after(0, self.kaydi_bitir_islem)
    ...
    def adb_getevent_dinle(self):
        ...
        def process_touch(slot_data):
            ...
            # --- İŞTE SADECE BURAYI DÜZELTTİM (Koordinat Kaymasını Yok Eden Kesin Matematik) ---
            ...
            if mesafe > 50: # 50 Pikselden fazla hareket ettiyse KESİN KAYDIRMADIR
                self.gecici_dokunuslar.append(f"S,{sx},{sy},{ex},{ey},{hareket_suresi_ms},{gecikme}")
                self.after(0, lambda: self.log_yaz(f"👆 Kaydırma: ({sx},{sy}) -> ({ex},{ey}) | Süre: {hareket_suresi_ms}ms"))
            else: # Aynı noktaya dokunduysa TIKLAMADIR
        ...
        while self.kayit_aktif:
            ...
            # 1. Hangi parmağın (slotun) verisi geliyor?
            if "ABS_MT_SLOT" in line:
                current_slot = int(line.split()[-1], 16)
                if current_slot not in slots: slots[current_slot] = {}
                
            # 2. Parmak Ekrana Dokundu mu / Çekildi mi?
            elif "ABS_MT_TRACKING_ID" in line:
                val = line.split()[-1]
                if current_slot not in slots: slots[current_slot] = {}
                
                if val == "ffffffff": # Parmak Ekrandan Çekildi! (Touch END)
                    if slots[current_slot].get('active'): 
                        process_touch(slots[current_slot])
                    slots[current_slot] = {'active': False}
                else: # Parmak Ekrana Dokundu! (Touch START)
                    slots[current_slot] = {'active': True, 'start_time': time.time(), 'start_x': None, 'start_y': None, 'end_x': None, 'end_y': None}
                    
            # 3. Bazı cihazlardaki alternatif Dokunma sinyali
                ...
                if val == "DOWN":
                    ...
                    if slots[0].get('active'): 
                        process_touch(slots[0])
                    slots[0] = {'active': False}
                    
            # 4. Parmağın X Koordinatı
            elif "ABS_MT_POSITION_X" in line:
                if current_slot not in slots: slots[current_slot] = {}
                if slots[current_slot].get('active'):
                    val = int(line.split()[-1], 16)
                    if slots[current_slot].get('start_x') is None: slots[current_slot]['start_x'] = val # İlk dokunduğu yer
                    slots[current_slot]['end_x'] = val # Parmağın güncel sürüklendiği yer
                    
            # 5. Parmağın Y Koordinatı
    ...
    def adim_ismi_sor_ve_kaydet(self):
        ...
        if case_adi:
            self.log_yaz(f"\n⏳ '{case_adi}' kaydediliyor, Referans Görüntü Çekiliyor...")
            self.update() 
    ...
    def arka_planda_case_kaydet(self, case_adi, dokunuslar):
        try:
            c_flags = subprocess.CREATE_NO_WINDOW if sys.platform == "win32" else 0
            ref_isim = f"ref_{self.guncel_test_adi.replace(' ', '_')}_{case_adi.replace(' ', '_')}_{int(time.time())}.png"
            ref_yol = os.path.join(self.referans_klasoru, ref_isim)
            
            subprocess.run([self.adb_yolu, "shell", "screencap", "-p", "/sdcard/ref.png"], capture_output=True, creationflags=c_flags)
            subprocess.run([self.adb_yolu, "pull", "/sdcard/ref.png", ref_yol], capture_output=True, creationflags=c_flags)
            ...
            self.after(0, lambda: self.log_yaz(f"✅ Adım '{case_adi}' BAŞARIYLA KAYDEDİLDİ! ({dokunus_sayisi} Eylem)"))
        except Exception as e:
    ...
    def goruntu_kiyasla_ve_isaretle(self, ref_yol, check_yol):
        try:
            img_ref_color = cv2.imread(ref_yol)
            img_check_color = cv2.imread(check_yol)
            ...
            cv2.imwrite(fark_yol, img_check_color)
        ...
        try:
            ...
            for dongu in range(1, tekrar_sayisi + 1):
                ...
                for c_adi, aksiyonlar, ref_veri in cases:
                    ...
                    test_foto_isim = f"temp_check_{int(time.time())}.png"
                    test_foto_yol = os.path.join(self.hata_klasoru, test_foto_isim)
                    
                    subprocess.run([self.adb_yolu, "shell", "screencap", "-p", "/sdcard/check.png"], capture_output=True, creationflags=c_flags)
                    subprocess.run([self.adb_yolu, "pull", "/sdcard/check.png", test_foto_yol], capture_output=True, creationflags=c_flags)
